...
4. **СТРОЖАЙШИЙ ЗАПРЕТ: НУЛЕВОЙ СЛЕД ПРИВАТНЫХ РЕПО НА РАБОЧЕМ НОУТБУКЕ (Zero Local Footprint):**
   - **Категорически запрещено сохранять, кэшировать или оставлять на диске рабочего ноутбука** любые данные, тексты, код, конфиги, файлы или результаты, если они получены из **любого приватного репозитория** (`Secret_Privat`, `Crypto_BOT`, `V-SaaP-Architecture` и др.) либо создаются для размещения в приватных репозиториях.
   - Любая работа с приватными репозиториями ведётся **исключительно в оперативной памяти ИИ и через прямой GitHub API**.
   - Никаких локальных файлов в `C:\CANCOM\`, `C:\UTIL\`, scratch-папках, temp или где-либо еще на диске `C:`.
   - Если в процессе неизбежно создавались временные скрипты или промежуточные файлы, они обязаны быть **немедленно и гарантированно удалены (wiped)** до отправки ответа.
   - Правило локального кэширования Столпа 1 распространяется **исключительно** на рабочие данные CANCOM (Jira, Confluence, публичные репозитории, технические логи). Приватные данные и приватные репозитории являются абсолютным исключением!
